...
func (h *Handler) ServeHTTP(rw http.ResponseWriter, req *http.Request) {
	...
	resp, err := h.Client.ContainerCreate(ctx, containerConfig, h.HostConfig, nil, "")
	if err != nil {
		internalError(err)
		return
	}
	containerID := resp.ID
	defer func() {
		status, err := h.Client.ContainerWait(ctx, containerID)
		if err != nil {
			h.printf("cgi: error getting container status: %s", err)
		}
		if status == 0 {
			...
			if err := h.Client.ContainerRemove(ctx, containerID, options); err != nil {
				h.printf("cgi: error removing container: %s", err)
			}
		}
	}()
	...
	attachResp, err := h.Client.ContainerAttach(ctx, containerID, attachOptions)
	if err != nil {
		internalError(err)
		return
	}
	...
	if err := h.Client.ContainerStart(ctx, containerID, containerStartOptions); err != nil {
		internalError(err)
		return
	}
	...
	// just write errors to stderr for now
	stderrWrite := os.Stderr
	...
	go func() {
		...
		if err := holdHijackedConnection(stdinRead, stdoutWrite, stderrWrite, attachResp); err != nil {
			h.printf("cgi: error in HoldHijackedConnection: %v", err)
		}
		stdoutWrite.Close()
		// stdoutWrite.Close()
	}()
	...
	if err != nil {
		...
		h.Client.ContainerKill(ctx, containerID, "KILL")
	}
}
